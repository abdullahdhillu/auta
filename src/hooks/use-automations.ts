...
export const useListener = (id: string) => {
  ...
  // ✅ Schema (same logic, just better typing)
  const promptSchema = z.object({
    prompt: z.string().min(1),
    reply: z.string().optional(), // matches your UI
  });
  ...
  const { isPending, mutate } = useMutationData(
    ...
    (data: FormValues) =>
      saveListener(
        ...
        listener,
        data.prompt,
        data.reply ?? "", // safe fallback
      ),
    "automation-info",
  );

  // ✅ Keep useZodForm usage EXACTLY same
  const { errors, onFormSubmit, register, reset, watch } = useZodForm(
    promptSchema,
    mutate,
  );
